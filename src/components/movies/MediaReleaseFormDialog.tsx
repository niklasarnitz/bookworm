...
import { createMediaReleaseSchema } from "~/schemas/video";
import { useRouter } from "next/navigation";
import type { z } from "zod";

type MediaReleaseFormData = z.infer<typeof createMediaReleaseSchema>;

interface MediaReleaseFormDialogProps {
  open: boolean;
...
export function MediaReleaseFormDialog({
  open,
  onOpenChange,
  movieId,
  movieTitle,
  release,
}: MediaReleaseFormDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const utils = api.useUtils();
  const router = useRouter();

  const form = useForm<MediaReleaseFormData>({
    resolver: zodResolver(createMediaReleaseSchema),
    defaultValues: {
      movieId,
  ...
  }, [open, release, form, movieId]);

  const createMutation = api.mediaRelease.create.useMutation({
    onSuccess: async () => {
      toast.success("Media release created successfully");
      form.reset();
      onOpenChange(false);
      // Invalidate and refetch movie data
  ...
  const updateMutation = api.mediaRelease.update.useMutation({
    onSuccess: async () => {
  ...
  const onSubmit = async (data: MediaReleaseFormData) => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {release ? "Edit Media Release" : "Add Media Release"}
          </DialogTitle>
          <p className="text-muted-foreground text-sm">
            {release
              ? `Edit the release "${release.editionName ?? "Standard Edition"}" for "${movieTitle}"`
              : `Add a new release or edition for "${movieTitle}"`}
          </p>
        </DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="e.g., Director's Cut, Special Edition"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
